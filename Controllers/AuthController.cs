    ...
    public class AuthController : ControllerBase
    {
        ...
        public IActionResult Register([FromForm] RegisterDTO model)
        {
            try
            {
                ...
                var user = _db.Users.FirstOrDefault(u => u.Email.ToLower() == model.Email.ToLower() && u.dlt == 0);
                if (user != null)
                {
                ...
                _db.Users.Add(oObject);
                var role = _db.Roles.FirstOrDefault(r => r.Name.ToLower() == model.Role.ToLower() && r.dlt == 0);
                if (role != null)
                {
                    ...
                    _db.UserRoles.Add(oObjectUserRole);
                }
                _db.SaveChanges();
        ...
        public IActionResult Login([FromForm] LoginDTO model)
        {
            try
            {
                ...
                var refreshToken = Helper.GeneratJwtToken.GenerateRefreshToken(user.UserId, _db);
        ...
        public IActionResult RefreshToken()
        {
            ...
            try
            {
                var oObjectRefreshToken = _db.RefreshTokens.FirstOrDefault(r => r.RefreshTokenSecret == refreshToken);
                ...
                {
                    var accessToken = Helper.GeneratJwtToken.GenerateAccessToken(user.UserId, user.Email, user.Roles[0]);
                    var newrefreshToken = Helper.GeneratJwtToken.GenerateRefreshToken(user.UserId, _db);
